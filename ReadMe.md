...
# Contents:  
- [Contents:](#contents)
- [0. Roborregos Candidates 2023](#0-roborregos-candidates-2023)
  - [Follow document instructions](#follow-document-instructions)
  - [Test with the challenge environment](#test-with-the-challenge-environment)
- [1. Introduction](#1-introduction)
- [2. Update Summary](#2-update-summary)
- [3. Preparations before using this package](#3-preparations-before-using-this-package)
  - [3.1 Install dependent package module](#31-install-dependent-package-module)
  - [3.2 Go through the official tutorial documents](#32-go-through-the-official-tutorial-documents)
  - [3.3 Download the 'table' 3D model](#33-download-the-table-3d-model)
  - [3.4 Install "mimic\_joint\_plugin" for xArm Gripper simulation in Gazebo](#34-install-mimic_joint_plugin-for-xarm-gripper-simulation-in-gazebo)
- [4. Getting started with 'xarm\_ros'](#4-getting-started-with-xarm_ros)
  - [4.1 Create a catkin workspace.](#41-create-a-catkin-workspace)
  - [4.2 Obtain the package](#42-obtain-the-package)
  - [4.2.1 update the package](#421-update-the-package)
  - [4.3 Install other dependent packages:](#43-install-other-dependent-packages)
  - [4.4 Build the code](#44-build-the-code)
  - [4.5 Source the setup script](#45-source-the-setup-script)
  - [4.6 First try out in RViz:](#46-first-try-out-in-rviz)
  - [4.7 Run the demo in Gazebo simulator](#47-run-the-demo-in-gazebo-simulator)
- [5. Package description \& Usage Guidance](#5-package-description--usage-guidance)
  - [5.1 xarm\_description](#51-xarm_description)
  - [5.2 xarm\_gazebo](#52-xarm_gazebo)
  - [5.3 xarm\_controller](#53-xarm_controller)
  - [5.4 xarm\_bringup](#54-xarm_bringup)
  - [5.5 xarm7\_moveit\_config](#55-xarm7_moveit_config)
      - [To run Moveit! motion planner along with Gazebo simulator:](#to-run-moveit-motion-planner-along-with-gazebo-simulator)
      - [To run Moveit! motion planner to control the real xArm:](#to-run-moveit-motion-planner-to-control-the-real-xarm)
      - [To run Moveit! motion planner to control the real xArm with xArm Gripper attached:](#to-run-moveit-motion-planner-to-control-the-real-xarm-with-xarm-gripper-attached)
      - [To run Moveit! motion planner to control the real xArm with xArm Vacuum Gripper attached:](#to-run-moveit-motion-planner-to-control-the-real-xarm-with-xarm-vacuum-gripper-attached)
  - [5.5.1 Add custom tool model for Moveit](#551-add-custom-tool-model-for-moveit)
    - [Examples:](#examples)
    - [Argument explanations:](#argument-explanations)
  - [5.6 xarm\_planner:](#56-xarm_planner)
      - [To launch the xarm simple motion planner together with the real xArm:](#to-launch-the-xarm-simple-motion-planner-together-with-the-real-xarm)
  - [5.7 xarm\_api/xarm\_msgs:](#57-xarm_apixarm_msgs)
      - [Robot Mode 0:](#robot-mode-0)
      - [Robot Mode 1:](#robot-mode-1)
      - [Robot Mode 4:](#robot-mode-4)
      - [Robot Mode 5:](#robot-mode-5)
      - [Robot Mode 6: (Firmware \>= v1.10.0)](#robot-mode-6-firmware--v1100)
      - [Robot Mode 7: (Firmware \>= v1.11.0)](#robot-mode-7-firmware--v1110)
      - [Starting xArm by ROS service:](#starting-xarm-by-ros-service)
      - [Joint space or Cartesian space command example:](#joint-space-or-cartesian-space-command-example)
        - [1. Joint space motion:](#1-joint-space-motion)
        - [2. Cartesian space motion in Base coordinate:](#2-cartesian-space-motion-in-base-coordinate)
        - [3. Cartesian space motion in Tool coordinate:](#3-cartesian-space-motion-in-tool-coordinate)
        - [4. Cartesian space motion in Axis-angle orientation:](#4-cartesian-space-motion-in-axis-angle-orientation)
        - [5. Joint velocity control:](#5-joint-velocity-control)
        - [6. Cartesian velocity control:](#6-cartesian-velocity-control)
      - [Motion service Return:](#motion-service-return)
      - [Tool I/O Operations:](#tool-io-operations)
        - [1. To get current 2 DIGITAL input states:](#1-to-get-current-2-digital-input-states)
        - [2. To get one of the ANALOG input value:](#2-to-get-one-of-the-analog-input-value)
        - [3. To set one of the Digital output:](#3-to-set-one-of-the-digital-output)
      - [Controller I/O Operations:](#controller-io-operations)
        - [1. To get one of the controller DIGITAL input state:](#1-to-get-one-of-the-controller-digital-input-state)
        - [2. To set one of the controller DIGITAL output:](#2-to-set-one-of-the-controller-digital-output)
        - [3. To get one of the controller ANALOG input:](#3-to-get-one-of-the-controller-analog-input)
        - [4. To set one of the controller ANALOG output:](#4-to-set-one-of-the-controller-analog-output)
      - [Getting status feedback:](#getting-status-feedback)
      - [Setting Tool Center Point Offset(only effective for xarm\_api ROS service control):](#setting-tool-center-point-offsetonly-effective-for-xarm_api-ros-service-control)
      - [Clearing Errors:](#clearing-errors)
      - [Gripper Control:](#gripper-control)
        - [1. Gripper services:](#1-gripper-services)
        - [2. Gripper action:](#2-gripper-action)
      - [Vacuum Gripper Control:](#vacuum-gripper-control)
      - [Tool Modbus communication:](#tool-modbus-communication)
      - ["report\_type" argument:](#report_type-argument)
...
- [7. xArm Vision](#7-xarm-vision)
  - [7.1 Installation of dependent packages:](#71-installation-of-dependent-packages)
    - [7.1.1 Install RealSense developer library and ROS package：](#711-install-realsense-developer-library-and-ros-package)
    - [7.1.2 Install 'aruco\_ros', for hand-eye calibration：](#712-install-aruco_ros-for-hand-eye-calibration)
    - [7.1.3 Install 'easy\_handeye', for hand-eye calibration：](#713-install-easy_handeye-for-hand-eye-calibration)
    - [7.1.4 Install 'vision\_visp' supporting package：](#714-install-vision_visp-supporting-package)
    - [7.1.5 Install 'find\_object\_2d', for object detection：](#715-install-find_object_2d-for-object-detection)
    - [7.1.6 Install other dependencies：](#716-install-other-dependencies)
    - [7.1.7 Build the whole workspace：](#717-build-the-whole-workspace)
  ...
  - [7.5 Color Cube Grasping Demo](#75-color-cube-grasping-demo)
    - [7.5.1 Download 'gazebo\_grasp\_plugin' for successful grasp simulation (ROS Melodic and later)](#751-download-gazebo_grasp_plugin-for-successful-grasp-simulation-ros-melodic-and-later)
    - [7.5.2 Gazebo grasping simulation (ROS Melodic and later)](#752-gazebo-grasping-simulation-ros-melodic-and-later)
    - [7.5.3 Real xArm and Intel realsense\_d435i hardware](#753-real-xarm-and-intel-realsense_d435i-hardware)
...
```bash
$ roslaunch xarm_gazebo retoAvanzados2023.launch
```
In another terminal, run the following to test the arm moving to clicked points on the camera image, as well as opening and closing the gripper. See the script to understand how the robot moves, how the clicked points translate to the robot moving goals and more:
```bash
$ rosrun xarm_gazebo demoReto2023.py
```

Check both the color cube grasping demo as well as the demoReto2023.py to understand how to move the robot and use the camera autonomously.
